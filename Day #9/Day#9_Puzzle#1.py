...
#########
histria = []
with open('Day#9_Puzzle#1_Input_Example.txt') as input_file:
...
for history in histria:
    ...
    for i in range(len(history)-2, -1, -1):
        sequence = history[i]
        sequence.append(sequence[-1] + history[i+1][-1])
print_histria()
...
###########################################
print("Sum of the Xtrapolated values is", sum([history[0][-1] for history in histria]))
